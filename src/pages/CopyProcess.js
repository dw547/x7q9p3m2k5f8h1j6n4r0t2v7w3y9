...
export default function CopyProcess() {
  ...
  const handleTickrateChange = (value) => {
    const newTickrate = Math.max(
      1,
      Math.min(1000, processValues.tick_rate + value)
    );
    setProcessValues({ ...processValues, tick_rate: newTickrate });
  };

  const fetchUAList = (osId, selectedUA) => {
    axios.get(`${process.env.REACT_APP_API_URI}/get-ua-by-os/${osId}`, {
      ...
      .then((response) => {
        ...
        // Set the selected UA after the list is fetched
        setProcessValues(prev => ({ ...prev, ua_list: selectedUA }));
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const handleOSChange = (e) => {
    ...
    setProcessValues(prev => ({
      ...
      ua_list: "",
      hardmask_type: "", // Reset hardmask_type when OS changes
    }));
  ...
  const handleServerChange = (e) => {
    if (e.target.value === "Add Server") {
      ...
      const selectedServer = availableServer.find(
        (server) => server.server_name === selectedServerName
      );
      setSelectedServer(selectedServerName);
      setProcessValues({
        ...processValues,
        server_list: selectedServer,
      });
    }
  };
      ...
      return null;
                ...
                <div className="flex flex-col sm:col-span-6">
                  <button
                    ...
                    className={` p-3 py-2 mb-2 w-full ${isButtonDisabled ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'
                      } border border-black rounded p-1 textwhite`}
                  >
                    Start Process
                    {isSubmitting && (
